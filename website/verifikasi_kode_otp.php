...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['email']) && isset($data['otp'])) {
        ...
        $stmt = $conn->prepare("SELECT otp, status_verifikasi FROM akun WHERE email = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if ($row['otp'] == $otp && $row['status_verifikasi'] == 0) {
                // Update status verifikasi menjadi 1 (terverifikasi)
                $update_stmt = $conn->prepare("UPDATE akun SET status_verifikasi = 1 WHERE email = ?");
